...
class hap_inf:
    ...
    def run_network(self,batch_a_haps,hap_lens,ort_constr):
        lat_h1, lat_h2,param_h1,param_h2,q_h1,q_h2= self.run_enc(train=True,units_firstlstm=self.units_firstlstm,batch_a_haps=batch_a_haps, hap_lens=hap_lens,reuse=None)
        sample_h1 = tf.exp(lat_h1)
        sample_h2 = tf.exp(lat_h2)
        pred_h1, pred_h2,intmdt_h1,intmdt_h2,intmdt2_h1,intmdt2_h2=self.run_decoder(hap_lens=hap_lens,reuse=None,inp_h1=sample_h1, inp_h2=sample_h2, dense_units=self.dense_units, units_bilstm=self.units_bilstm, units_bilstm2=self.units_bilstm2)
    ...
    def test_network(self,batch_a_haps,hap_lens):
        lat_h1, lat_h2,param_h1,param_h2,q_h1,q_h2= self.run_enc(hap_lens=hap_lens,train=False,units_firstlstm=self.units_firstlstm,batch_a_haps=batch_a_haps, reuse=True)
        pred_h1, pred_h2,_,_,_,_ =self.run_decoder(hap_lens=hap_lens,reuse=True,inp_h1=lat_h1, inp_h2=lat_h2, dense_units=self.dense_units, units_bilstm=self.units_bilstm, units_bilstm2=self.units_bilstm2)
