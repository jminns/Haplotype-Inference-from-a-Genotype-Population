...
def train(**kwargs):
    ...
    alt_rnn=kwargs['alt_rnn']
    dense_intmdt_pred_units=kwargs['dense_intmdt_pred_units']
    lmbda_next = kwargs['lmbda_next']
    ...
    net = hap_inf(lmbda_next=lmbda_next,dense_intmdt_pred_units=dense_intmdt_pred_units,alt_rnn=alt_rnn,max_hap_len=max_hap_len, units_bilstm2=units_bilstm2, units_bilstm=units_bilstm, dense_units=dense_units, units_firstlstm=units_firstlstm,
    ...
    test_cost, test_reconstruction, full_kl_test, pred_h1_test, pred_h2_test = net.test_alt_network(batch_a_haps=batch_a_haps_test_pl, hap_lens=hap_lens_pl, ort_constr=ort_constr)
    ...
    for count in range(num_epochs):
        ...
        for batch in n_batches_inds:
            ...
            print('pred h1 {}\n pred h2 {} \n true h1 {} \n true h2 {}'.format(pred_h1_train_o[0][0:10],pred_h2_train_o[0][0:10],hap1_true_train[batch][0][0:10],hap2_true_train[batch][0][0:10]))
            summary_writer.add_summary(summary_inf_train_o, global_step_o)
            summary_writer.flush()
            if global_step_o % 100 == 0:
                sess.run(tf.local_variables_initializer())
                r_num = np.random.randint(low=0,high=np.shape(test_haps_mat)[0])
                full_kl_test_o,test_cost_o, summary_inf_test_o = sess.run([full_kl_test,test_cost, summary_inf_test],feed_dict={hap_lens_pl:lens,batch_a_haps_test_pl:test_haps_mat[r_num],true_hap1_pl_test:hap1_true_test[r_num],true_hap2_pl_test:hap2_true_test[r_num]})
                print('Test Cost {} test Kl {}'.format(test_cost_o,full_kl_test_o))
...
if __name__=='__main__':
    loc = argv[-1]
    dict ={'alt_rnn':True,'dense_intmdt_pred_units':128,'lmbda_next':1,'KL':False,'ort_constr':False,'num_epochs':100,'num_ancs':10,'dim_ancestors':128,'dense_units':128,'units_firstlstm':128,'units_bilstm2':256,'units_bilstm':256,'max_hap_len':143,'batch_size':100,'prior_temp':0.5,'post_temp':0.5,'log_dir':loc,'log_file':'256x_sampling_10ancs_tmp05'}
